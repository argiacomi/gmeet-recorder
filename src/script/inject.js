...
const GoogleMeets = (audioChunks, webRtcProxy, rtcSenderProxy, config = {}) => {
  ...
  const startAudioRecording = async (timeslice) => {
    ...
    try {
      ...
      if (audioChunkId !== START_CHUNK_ID) {
        logger.error(`'[StartAudioRecording] Incorrect audio ID. Got: ${audioChunkId}, Expected: ${START_CHUNK_ID}`);
  ...
  const toggleRecording = () => {
    if (isRecording && audioRecorder) {
      isPaused ? audioRecorder.resume() : audioRecorder.pause();
      isPaused ? logger.debug('Recording resumes') : logger.debug('Recording paused');
      isPaused = !isPaused;
    } else {
      logger.warn('Cannot toggle: not recording or already paused');
    }
  };
  ...
  const handleMessage = (event) => {
    ...
    switch (event.data?.type) {
      case EVENTS.START_RECORDING:
        ...
        break;
      case EVENTS.TOGGLE_RECORDING:
        logger.info('[GoogleMeets] Toggle recording requested');
        window.googleMeets.toggleRecording();
        break;
